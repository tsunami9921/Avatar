...
local player = Players:FindFirstChild("babaprmv")
...
task.spawn(function()
	...
	loadingLabel.Text = "Loading TSU Avatar..."
	loadingLabel.TextStrokeTransparency = 0.5

	task.spawn(function()
		while loadingFrame.Parent do
			for i=1,20 do
				loadingLabel.TextColor3 = Color3.fromHSV(i/20,1,1)
				task.wait(0.05)
			end
		end
	end)
	...
	wait(2)
	tween(loadingFrame,{BackgroundTransparency=1},0.5):Play()
	task.wait(0.5)
	...
	for i=1,3 do
		extraButtons[i].Parent = rightPanel
	...
	minimizeBtn.MouseButton1Click:Connect(function() main.Visible=false tsuIcon.Visible=true end)
	tsuIcon.MouseButton1Click:Connect(function() tsuIcon.Visible=false main.Visible=true end)

	local function animateSearch()
	...
	search.MouseButton1Click:Connect(function()
		...
		if thumb then img.Image=thumb img.Visible=true end
		loading.Text="✅ "..name.." found!"
		loading.Visible=true task.delay(2,function() loading.Visible=false end)
		for i=1,3 do buttons[i].Visible=true end
		buttons[1].MouseButton1Click:Connect(function() createRig(userId,"R6",buttons[1]) end)
		buttons[2].MouseButton1Click:Connect(function() createRig(userId,
